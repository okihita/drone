...
export default function AccountabilityHubPage() {
  return (
    <>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 w-full font-sans space-y-12 sm:space-y-16">
        {/* Monitored Ecosystem Callout */}
        <section className="p-8 sm:p-10 rounded-3xl bg-slate-100/70 dark:bg-slate-900/60 border border-slate-200/70 dark:border-slate-800/80 space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <span className="text-sm font-bold uppercase tracking-wider text-asean-blue">Corporate AI Telemetry</span>
              <h2 className="text-xl sm:text-2xl font-bold font-serif-editorial text-slate-900 dark:text-white mt-1">
                Regional Platform Accountability Framework
              </h2>
            </div>
            <span className="inline-flex items-center text-sm font-semibold px-3.5 py-1.5 rounded-full bg-asean-amber/15 text-asean-amber border border-asean-amber/30 w-fit">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6">
            {TARGET_PLATFORMS.map((platform) => (
              <div
                key={platform.name}
                className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-700/80 shadow-2xs flex flex-col justify-between space-y-3"
              >
                <div>
                  <div className="font-bold text-base text-slate-900 dark:text-white">{platform.name}</div>
                  <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">{platform.role}</div>
                </div>
                <div className="text-sm font-semibold text-asean-blue pt-3 border-t border-slate-100 dark:border-slate-700/50">
                  {platform.focus}
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
            {PILLAR_MODULES.map((mod) => (
              <Link
                key={mod.href}
                href={mod.href}
                className={`group flex flex-col justify-between p-8 sm:p-10 rounded-3xl bg-white dark:bg-slate-900/60 border border-slate-200/70 dark:border-slate-800/80 ${mod.accent} shadow-xs hover:shadow-md transition-all duration-200`}
              >
                <div className="space-y-4">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-asean-yellow transition-colors">
                    {mod.title}
                  </h3>
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-sm font-semibold text-slate-500 dark:text-slate-400">
