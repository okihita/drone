...
export default function NewsList() {
  ...
  const fetch = useCallback(() => {
    (async () => {
      const { data, error: fetchErr } = await supabase.from("news_items").select("id,title,jurisdiction,category,image_url,published_date").order("published_date", { ascending: false });
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error: delErr } = await supabase.from("news_items").delete().eq("id", id);
  ...
  return (
    <AdminDashboardLayout>
      ...
      <div className="rounded-lg border bg-white dark:bg-slate-900">
        <Table>
          <TableHeader><TableRow><TableHead className="w-16">Image</TableHead><TableHead>Title</TableHead><TableHead>Jurisdiction</TableHead><TableHead>Category</TableHead><TableHead>Published</TableHead><TableHead className="w-20">Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {loading ? Array.from({ length: 3 }).map((_, i) => <TableRow key={i}><TableCell colSpan={6}><Skeleton className="h-10 w-full" /></TableCell></TableRow>)
              : items.map(n => (
                <TableRow key={n.id}>
                  <TableCell>{n.image_url ? <Image src={n.image_url} alt="" width={48} height={32} className="rounded object-cover w-12 h-8" /> : <div className="w-12 h-8 rounded bg-muted flex items-center justify-center"><Newspaper className="w-3 h-3 text-muted-foreground" /></div>}</TableCell>
