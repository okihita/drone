...
export default function Header() {
  ...
  return (
    <header className="w-full border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090d16] transition-colors">
      {/* Top Editorial Masthead Bar */}
      <div className="border-b border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950 px-4 py-2 text-[11px] text-slate-600 dark:text-slate-400 font-mono-data">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 border-b border-slate-200 dark:border-slate-800/60">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="hidden md:flex items-center gap-8 py-3 text-xs font-medium text-slate-700 dark:text-slate-300">
          <Link href="#featured-analysis" className="hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5 transition-colors">
            <BookOpen className="w-3.5 h-3.5 text-amber-600 dark:text-amber-500" />
            <span>Featured Analysis</span>
          </Link>
          <Link href="#asean-map" className="hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5 transition-colors">
            <Map className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
            <span>ASEAN Map &amp; Dossiers</span>
          </Link>
          <Link href="#policy-ledger" className="hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5 transition-colors">
            <Database className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
            <span>Policy Ledger &amp; Table</span>
          </Link>
          <Link href="#defa-monitor" className="hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5 transition-colors">
            <FileText className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" />
            <span>DEFA Monitor</span>
          </Link>
          <Link href="#threat-matrix" className="hover:text-amber-600 dark:hover:text-amber-400 flex items-center gap-1.5 transition-colors">
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-4 py-4 space-y-3 text-xs">
          <Link href="#featured-analysis" onClick={() => setMobileMenuOpen(false)} className="block py-1 text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400">
            Featured Analysis
          </Link>
          <Link href="#asean-map" onClick={() => setMobileMenuOpen(false)} className="block py-1 text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400">
            ASEAN Map &amp; Dossiers
          </Link>
          <Link href="#policy-ledger" onClick={() => setMobileMenuOpen(false)} className="block py-1 text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400">
            Policy Ledger &amp; Data Table
          </Link>
          <Link href="#defa-monitor" onClick={() => setMobileMenuOpen(false)} className="block py-1 text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400">
            DEFA Monitor
          </Link>
          <Link href="#threat-matrix" onClick={() => setMobileMenuOpen(false)} className="block py-1 text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400">
