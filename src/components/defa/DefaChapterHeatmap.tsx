  ...
  return (
    <>
      <HeroBanner
        ...
        rightSlot={
          <div className="shrink-0 p-6 sm:p-7 rounded-2xl bg-slate-100/80 dark:bg-slate-800/70 border border-slate-200/70 dark:border-slate-800/80 shadow-xs text-sm font-sans space-y-4 min-w-[290px]">
            ...
            <div className="flex items-center justify-between text-sm text-slate-500 pt-2 border-t border-slate-200/70 dark:border-slate-700/80">
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 font-sans">
        <div className="max-w-7xl mx-auto space-y-10 sm:space-y-12">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 sm:gap-6 p-6 sm:p-7 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200/70 dark:border-slate-800/80 shadow-xs">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm font-sans">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-semibold bg-asean-emerald/15 text-asean-emerald border-asean-emerald/40">
                <CheckCircle2 className="w-3.5 h-3.5" /> Concluded (100%)
              </span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-semibold bg-asean-blue/15 text-asean-blue border-asean-blue/40">
                <Clock className="w-3.5 h-3.5" /> Legal Scrubbing (75-90%)
              </span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-semibold bg-asean-amber/15 text-asean-amber border-asean-amber/40">
                <AlertTriangle className="w-3.5 h-3.5" /> Provisional Reservation (60-70%)
              </span>
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-semibold bg-asean-red/15 text-asean-red border-asean-red/40">
                <Lock className="w-3.5 h-3.5" /> Pending Consultation (&lt;50%)
              </span>
            </div>
          </div>

          {/* 11 Member State x 9 Chapter Heatmap Grid */}
          <div className="overflow-x-auto rounded-2xl border border-slate-200/70 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-xs">
            <table className="w-full border-collapse text-left text-sm font-sans min-w-[1050px]">
              <thead>
                <tr className="border-b border-slate-200/70 dark:border-slate-800/80 bg-slate-100/80 dark:bg-slate-950/80 text-slate-700 dark:text-slate-300 uppercase tracking-wider font-bold">
                  <th className="p-4 sticky top-0 left-0 z-30 bg-slate-100 dark:bg-slate-950 min-w-[160px] max-w-[180px] border-r border-slate-200/70 dark:border-slate-800/80 shadow-[2px_0_6px_rgba(0,0,0,0.04)]">
                    ASEAN Nation
                  </th>
                  {DEFA_CHAPTERS.map((ch) => (
                    <th key={ch.id} className="p-3.5 text-center min-w-[130px] sm:min-w-[145px] max-w-[160px] align-bottom sticky top-0 z-20 bg-slate-100 dark:bg-slate-950 shadow-[0_2px_4px_rgba(0,0,0,0.04)]">
                      <div className="flex flex-col items-center gap-1 mb-1.5">
                        <span className="inline-block px-2 py-0.5 rounded bg-asean-blue/10 dark:bg-asean-blue/20 text-asean-blue dark:text-asean-yellow font-sans text-sm font-extrabold">
                          {ch.code}
                        </span>
                        <span className={`inline-block px-2 py-0.5 rounded text-sm font-bold border ${getCivilSocietyRiskBadge(ch.civilSocietyRisk)}`}>
                          {ch.civilSocietyRisk}
                        </span>
                      </div>
              ...
              <tbody className="divide-y divide-slate-200/70 dark:divide-slate-800/80">
                {ASEAN_MEMBER_STATES.map((country) => (
                  <tr key={country.code} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="p-3.5 sticky left-0 z-10 bg-white dark:bg-slate-900 font-bold text-slate-900 dark:text-white border-r border-slate-200/70 dark:border-slate-800/80 min-w-[160px] max-w-[180px] shadow-[2px_0_6px_rgba(0,0,0,0.04)] align-middle">
                      <div className="flex items-center gap-2.5">
                      ...
                      return (
                        <td key={ch.id} className="p-1 sm:p-1.5 text-center align-stretch">
                          <button
                            ...
                            className={`w-full h-full min-h-[58px] py-2 px-1.5 rounded-lg border flex flex-col items-center justify-between transition-all ${badge.className} ${
          ...
          {selectedCell && activeChapter && activeCountry && activeStatus && (
            <div className="p-6 sm:p-8 rounded-2xl bg-white dark:bg-slate-900/95 border border-slate-200/70 dark:border-slate-800/80 shadow-lg space-y-6 sm:space-y-8 animate-[fadeIn_0.2s_ease-out]">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200/70 dark:border-slate-800/80 pb-4">
                <div className="flex items-center gap-3">
                  <activeCountry.Flag className="w-7 h-5 object-cover rounded-[2px] shadow-xs" />
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-bold border ${getCivilSocietyRiskBadge(activeChapter.civilSocietyRisk)}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 text-sm font-sans">
                <div className="p-5 sm:p-6 rounded-xl bg-slate-50/70 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 space-y-2">
                  <span className="font-sans text-sm uppercase font-bold text-slate-500 tracking-wider">SEOM Chapter Objective</span>
                  <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{activeChapter.description}</p>
                </div>

                <div className="p-5 sm:p-6 rounded-xl bg-slate-50/70 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 space-y-2">
                  <span className="font-sans text-sm uppercase font-bold text-asean-blue tracking-wider">TPP / International Trade Comparison</span>
                  <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{activeChapter.tppComparison}</p>
                </div>

                <div className="p-5 sm:p-6 rounded-xl bg-slate-50/70 dark:bg-slate-950/70 border border-slate-200/70 dark:border-slate-800/80 space-y-2">
                  <span className="font-sans text-sm uppercase font-bold text-asean-amber tracking-wider">Gazette Citation &amp; Status Note</span>
                  <p className="font-sans text-sm font-bold text-slate-900 dark:text-white">{activeStatus.gazetteCitation}</p>
                  <p className="text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">{activeStatus.notes}</p>
                </div>
              </div>
            </div>
          )}
