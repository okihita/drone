      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6">
        {entries.map(([code, data]) => {
          ...
          return (
            <div
              key={code}
              className="p-5 sm:p-6 rounded-2xl border border-slate-200/70 dark:border-slate-800/80 bg-white dark:bg-slate-900 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    {(() => {
                      ...
                      return FlagIcon ? <FlagIcon className="w-4 h-3 rounded-[2px]" /> : null;
                    })()}
                    <span className="font-sans text-sm font-extrabold text-slate-400 tracking-wider">{code}</span>
                  </div>
              ...
              <div className="mt-3.5 h-2 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
        ...
        {entries.length === 0 && (
          <div className="col-span-full text-center py-12 text-sm text-slate-400">No encryption events tracked.</div>
        )}
      </div>
    </section>
  );
