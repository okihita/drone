    ...
    <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-b border-slate-200 dark:border-slate-800">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-6 lg:border-r lg:border-slate-200 lg:dark:border-slate-800 lg:pr-8 animate-fade-up [animation-delay:150ms]">
          ...
          {FIELD_DISPATCHES.map((dispatch, idx) => (
            <article
              key={dispatch.title}
              className={`group space-y-3 ${idx < FIELD_DISPATCHES.length - 1 ? "pb-6 border-b border-slate-200 dark:border-slate-800" : ""}`}
            >
        ...
        <div className="lg:col-span-3 space-y-6 animate-fade-up [animation-delay:300ms]">
          ...
          <div className="space-y-4 font-sans">
            {REGULATORY_RADAR.map((entry) => {
              const alertColor = entry.alertLevel === "High Alert" ? "text-asean-red" : "text-asean-blue";
              return (
                <div
                  key={entry.title}
                ...
                >
                  <div className="flex items-center justify-between text-[10px] font-sans">
                    ...
                    <span className={`font-bold ${alertColor}`}>{entry.alertLevel}</span>
                  </div>
          ...
          >
            <span>Explore Full Policy Ledger ({REGULATORY_RADAR.length})</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
      </div>
