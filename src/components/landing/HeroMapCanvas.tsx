...
export default function HeroMapCanvas({
  ...
  return (
    ...
    >
            ...
            return (
              <g key={country.id} className="cursor-pointer transition-all duration-300">
                ...
                <text
                  ...
                  className={`pointer-events-none font-sans text-sm font-bold tracking-wider transition-all duration-300 ${
                    isSelected ? "fill-asean-yellow text-sm" : "fill-slate-700 dark:fill-slate-300"
                  }`}
                  style={{
      ...
      <div className="absolute right-4 top-4 z-30 flex flex-col items-end gap-2 font-sans text-sm sm:right-6 sm:top-6">
        <span className="hidden sm:inline-block text-sm font-sans font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400/90 pr-1">
          {FLOW_ARCS.length} Data Flow Corridors Monitored
        </span>

        {/* ASEAN Regime Posture Card */}
        <div className="flex flex-col items-stretch gap-1 rounded-xl border border-slate-200 bg-white/90 p-2 font-sans text-sm shadow-xs backdrop-blur-md min-w-[130px] dark:border-white/20 dark:bg-slate-950/85">
          <span className="text-sm font-sans uppercase tracking-widest text-slate-500 dark:text-slate-400 font-bold px-0.5">
            ASEAN Regime Posture
          </span>
          <div className="flex items-center justify-between gap-1 text-sm font-sans font-bold pt-0.5">
        ...
        {onSelectLayer && (
          <div className="flex flex-col items-stretch gap-1 rounded-xl border border-slate-200 bg-white/90 p-1.5 shadow-xs backdrop-blur-md min-w-[130px] dark:border-white/20 dark:bg-slate-950/85">
            <span className="flex items-center gap-1 px-2 py-0.5 text-sm font-sans font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
          ...
          {countries.map((c) => {
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-1.5 rounded-lg border px-3 py-2 text-sm font-bold transition-all ${
                  isSelected
              ...
              >
                {FlagIcon ? (
                  ...
                  <span className="font-sans font-bold text-sm">[{c.code}]</span>
                )}
                <span className="font-sans font-bold text-sm">{c.code}</span>
                {c.threatScore >= 4 && (
      ...
      <div className="pointer-events-none absolute left-4 bottom-3 z-20 font-sans text-sm uppercase font-bold tracking-widest text-slate-500/90 dark:text-slate-500/80 sm:left-6 sm:bottom-3.5 space-y-0.5">
